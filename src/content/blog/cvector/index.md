...
I decided that I would use a ralph wiggum loop overnight to build the project, being inspired by Geoffery Huntley's article [Ralph](https://ghuntley.com/ralph/).

> The idea behind ralph is when we are engineering a series of features we break them down into incremental steps each with its own user story and acceptance criteria, and we are going to run LLM sessions where each time it tries to work on one feature, document its progress until all tasks are accepted. If a ralph iteration (1 LLM session) doesn't complete one task, it's going to write it's progress on that task into a file, so the next run can pick it up.
...
I chatted with Claude about how to do the frontend and data base design, after feeding in the context about the project.

![Asking Claude for frontend designs](./claude-designs.png)

*I asked it to give 3-4 frontend designs and settled down on one of them, asked it to use light mode and make the UI more legible (since we are using it for industrial control purposes, readability is key)*

![Iterating on the UI design](./ui-iteration.png)

Then I fine-tuned how certain parts of the UI should look like to enhance readability, wanting it to be functional and distraction-free.
...
I supplemented that with SQLAlchemy as the ORM (the standard pairing with FastAPI), SQLite for portability because it's just a take-home exam, Recharts for time-series visualization (Ant Design doesn't ship its own charting library), and Vite as the build tool.
...
We ended up completing all the tasks just in a few ralph sessions, and here's the completed product:
